...
function detectJailbreak(prompt) {
    ...
    for (const keyword of constants_1.JAILBREAK_KEYWORDS) {
    ...
    for (const regex of constants_1.JAILBREAK_PATTERNS) {
    ...
    if ((0, utils_1.detectExcessiveSpacing)(prompt)) {
        matchedRules.push('Obfuscation: Excessive spacing detected');
        score += 0.3;
    }
    for (const regex of constants_1.OBFUSCATION_PATTERNS) {
